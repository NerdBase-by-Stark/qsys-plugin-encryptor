...
rem -- QSC tool: our fork, pinned to a commit (keep in sync with the workflow)
set "QSC_TOOL_REPO=https://github.com/NerdBase-by-Stark/PluginEncryptionTool.git"
...
echo [3/6] Fetching QSC encryption tool (pinned fork) ...
if not exist "vendor\plugin-tool\plugin_tool_release.exe" (
    ...
    git clone --quiet %QSC_TOOL_REPO% "%TEMP%\qsc-tool" || goto :fail
...
echo [4/6] Verifying tool checksums ...
python verify_tool.py || goto :fail
